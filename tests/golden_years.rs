//! 本家 jpholiday（Python）の年別ゴールデンテスト（1971-2027）を決定的に移植したもの。
//!
//! 上流リポジトリの `tests/test_year_*.py` に含まれる全アサーションを、3 種類の規則的な
//! 検査（祝日名・月別件数・年別件数）として 1 対 1 に書き起こしている。本家の期待値を
//! そのまま固定したゴールデンデータであり、手で編集しないこと。
